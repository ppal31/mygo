...
type AppConfig struct {
	...
	Database struct {
		...
		Datasource string `envconfig:"APP_DATABASE_DATASOURCE" default:"database.sqlite3?_foreign_keys=on"`
	}

	// Token defines token configuration parameters.
	Token struct {
		JWTAuthSecret []byte        `envconfig:"APP_TOKEN_JWT_SECRET" required:"true"`
